...
public class BestScoreLeastCrowded implements IAgentSelectionStrategy {
    double epsilon;
    private Map<IDAgent, OCEMessage> oceServiceAgentPerceptionHistory;

    public BestScoreLeastCrowded(double epsilon, Map<IDAgent, OCEMessage> oceServiceAgentPerceptionHistory) {
        this.epsilon = epsilon;
        this.oceServiceAgentPerceptionHistory = oceServiceAgentPerceptionHistory;
    }
    ...
    public Optional<Map.Entry<IDAgent, ScoredCurrentSituationEntry>> execute(Situation<ScoredCurrentSituationEntry> scoredCS) {
        ...
        if(explorationProbability <= epsilon){ // Exploration : choose randomly an agent (which is probably not the best one)
            ...
            if(listBestScoreAgent.size()>1){ // Refer to the priority defined by the ARSA protocol
                ...
                int HighestPriority  = (int) scoredCS.getAgentSituationEntries().values().stream().
                        filter(e-> listBestScoreAgent.contains(e.getAgent())).map(e-> e.getMessageType().ordinal()).
                        max(Comparator.comparing(Integer::intValue)).get();
                System.out.println("MAX Priority = "+HighestPriority);
                List<IDAgent> listAgentHighestPriority = scoredCS.getAgentSituationEntries().values().stream().
                        filter(e-> listBestScoreAgent.contains(e.getAgent())).filter(e -> e.getMessageType().ordinal()==HighestPriority).
                        map(e -> e.getAgent()).collect(Collectors.toList());
                System.out.println("Liste des elements Highest Priority : " + listAgentHighestPriority.size());
                System.out.println("Liste des elements Highest Priority : " + listAgentHighestPriority.toArray().toString());
              //  List<IDAgent> listAgentsLeastCrowded = oceServiceAgentPerceptionHistory.values().stream().filter(e -> ((ServiceAgent) e.getEmitter()).getHandledService().getServiceQalities().get(0).getValue(41).equals("Low")).map(e -> e.getEmitter().getMyID()).collect(Collectors.toList());
                List<IDAgent> listAgentsLeastCrowded = oceServiceAgentPerceptionHistory.values().stream().
                        filter(e -> ((ServiceAgent) e.getEmitter()).getHandledService().getCrowdedness().equals("Low")).map(e -> e.getEmitter().getMyID()).collect(Collectors.toList());
                System.out.println("Liste des élement least Crowded : " + listAgentsLeastCrowded.size());
                System.out.println("Liste des élement least Crowded : " + listAgentsLeastCrowded.toArray().toString());
                List<IDAgent> listHighPriorityLeastCrowded = listAgentHighestPriority.stream().distinct().
                        filter(listAgentsLeastCrowded::contains).collect(Collectors.toList());

                List<IDAgent>  finalList;
                if(listAgentsLeastCrowded.isEmpty()){
                    finalList = listAgentHighestPriority;
                }
                else{
                    finalList = listAgentsLeastCrowded;
                }
                if(finalList.size()>1){ // If their is multiple agents with the best score having the same priority -> choose randomly
                    chooseLater = true; // mark that we will need to choose later the best agent;
                    listAgentsToChooseFrom = Optional.ofNullable(finalList);
                }else{
                    ...
                    bestAgent = Optional.ofNullable(finalList.get(0));
                }
            }else{
                chooseLater= false; // mark that we won't need to choose later;
                //we take the first element cause we are sure that their is only one element
