...
public class SelectCriteria implements Criteria {
    ...
    /**
     * This function is used to filter a list of messages to keep only the select message
     * @param messages : the list of messages to filter
     * @return the filtered list containing only the select messages
     */
    @Override
    public ArrayList<Message> meetCriteria(ArrayList<Message> messages) {
        return new ArrayList<Message>( messages.stream().filter(m -> m.getMyType()== MessageTypes.SELECT)
